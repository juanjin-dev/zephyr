...
#if WIZNET_ANY_INT_GPIOS
#define WIZNET_INT_INIT(node) .interrupt = GPIO_DT_SPEC_GET_OR(node, int_gpios, {0}),
#else
#define WIZNET_INT_INIT(node)
#endif
...
#define WIZNET_DEVICE_DEFINE(node, init_fn, api, phy_api, chip_ops, chip_regs, stack_size, prio,   \
	...
	static const struct wiznet_config wiznet_config_##node = {                                 \
		...
		WIZNET_INT_INIT(node).reset = GPIO_DT_SPEC_GET_OR(node, reset_gpios, {0}),         \
