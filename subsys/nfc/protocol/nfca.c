...
int z_nfca_transceive(struct nfc_poller *poller, const struct nfc_target *target, const uint8_t *tx,
		      uint16_t tx_len, bool tx_crc, bool rx_crc, uint8_t *rx, uint16_t *rx_len,
		      uint32_t timeout_us)
{
	uint8_t frame[32];
	uint16_t len = tx_len;
	struct nfc_property props[] = {
		{.type = NFC_PROP_HW_TX_CRC, .hw_tx_crc = tx_crc},
		{.type = NFC_PROP_HW_RX_CRC, .hw_rx_crc = rx_crc},
		{.type = NFC_PROP_TIMEOUT, .timeout_us = timeout_us},
	};
	bool sw_rx_crc;
	int ret;
	...
	if (z_nfc_poller_backend(poller) == Z_NFC_BACKEND_OFFLOAD) {
		...
		return nfc_offload_exchange(poller->dev, target, tx, tx_len, rx, rx_len,
					    DIV_ROUND_UP(timeout_us, USEC_PER_MSEC));
	}
	...
	if (tx_crc && props[0].status == -ENOTSUP) {
		nfc_iso14443a_crc_append(frame, len);
		len += 2U;
	}
	sw_rx_crc = rx_crc && (props[1].status == -ENOTSUP);
...
int z_nfca_halt(struct nfc_poller *poller, const struct nfc_target *target, k_timeout_t timeout)
{
	...
	ret = z_nfca_transceive(poller, target, cmd, sizeof(cmd), true, false, rx, &rx_len,
				z_nfc_timeout_us_cap(sys_timepoint_calc(timeout), 1100U + 60U));
